...
//
// Tách khỏi page.tsx (2026-09-03) để page.tsx có thể là Server Component
// export generateMetadata (Open Graph title/ảnh bìa theo từng tour, xem
// page.tsx cùng thư mục) — "use client" không được export generateMetadata.
//
...
export function TourPublicClient({ tourId }: { tourId: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#f7f6f3]">
        ...
        <div className="mx-auto flex h-16 max-w-3xl items-center justify-between px-4">
          ...
          <div className="flex shrink-0 items-center gap-3">
            {company?.logo_url && (
              ...
              <img src={`${API_BASE}${company.logo_url}`} alt="" className="hidden h-8 w-8 object-contain md:block" />
            )}
            <span className="hidden text-xl font-bold text-[#1a6b4e] md:block">{company?.name ?? "VietTour"}</span>
            <button
              type="button"
              onClick={onShare}
      ...
      <footer className="mb-16 flex w-full flex-col items-center gap-2 border-t border-[#e5e5e5] bg-[#f7f6f3] py-8 text-center md:mb-0">
        {company?.logo_url && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={`${API_BASE}${company.logo_url}`} alt="" className="h-10 w-10 object-contain" />
        )}
        <div className="text-xl font-bold text-[#1a6b4e]">{company?.name ?? "VietTour Agent"}</div>
